...
const Checkout = () => {
  ...
  useEffect(() => {
    ...
    if (paymentStatus) {
      ...
      if (paymentStatus === 'success' && sessionId) {
        axios.post(`${import.meta.env.VITE_BASE_URL}/api/orders/confirm`,
          { sessionId },
          { headers: authHeaders })
          .then(({ data }) => {
            clearCart();
  ...
  return (
    <div className='min-h-screen bg-gradient-to-b from-[#1a1212] to-[#2a1e1e] text-white
    py-16 px-4'>
      <div className='mx-auto max-w-4xl'>
        ...
        <form onSubmit={handleSubmit} className='grid lg:grid-cols-2 gap-12'>
          <div className="bg-[#4b3b3b]/80 p-6 rounded-3xl space-y-6" >
            ...
            <Input label="Email" name="email" type="email" value={formData.email} onChange={handleInputChange} />                       <Input label="Address" name="address" value={formData.address} onChange={handleInputChange} />
...
const PaymentSummary = ({ totalPrice }) => {
  const safeAmount = typeof totalPrice === 'number' ? totalPrice : 0;
  const subtotal = Number(safeAmount.toFixed(2));
  const tax = Number((subtotal * 0.05).toFixed(2));
