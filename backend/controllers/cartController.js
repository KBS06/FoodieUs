...
export const addToCart = asyncHandler(async (req, res) => {
  ...
  if (!itemId || typeof quantity !== "number" || quantity < 0) {
    res.status(400);
    throw new Error("itemId and quantity (non-negative) are required");
  }
  ...
  // Don't add a new item with quantity < 1
  if (quantity < 1) {
    return res
      .status(400)
      .json({ message: "Cannot add item with quantity < 1" });
  }
...
//LETS A CREATE A METHOD TO UPDATE OUR CART AND ITEMS QUANTITY
